...
void Simulator::update(float deltaTime, Profiler &profiler)
{
	...
	for (int i = 0; i < bodiesSize; i++)
	{
		...
		//gravity
		bodies[i].acceleration += glm::vec3{0, -9.81, 0};
		...
		updateForces(bodies[i], deltaTime);

	#pragma region hit walls
		auto minPos = bodies[i].getMin();
		auto maxPos = bodies[i].getMax();
